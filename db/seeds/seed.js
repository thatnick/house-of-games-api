...
const seed = async (data) => {
  const { categoryData, commentData, reviewData, userData, voteData } = data;
  ...
  const insertReviewsQueryStr = format(
    "INSERT INTO reviews (title, category, designer, owner, review_body, review_img_url, created_at) VALUES %L RETURNING *;",
  ...
  const insertVotesQueryStr = format(
    "INSERT INTO votes ( username, review_id ) VALUES %L RETURNING *;",
    voteData.map(({ username, review_id }) => [username, review_id])
  );
  await db.query(insertVotesQueryStr).then((result) => result.rows);
  ...
  const insertCommentsQueryStr = format(
    "INSERT INTO comments (body, author, review_id, created_at) VALUES %L RETURNING *;",
    formattedCommentData.map(({ body, author, review_id, created_at }) => [
      body,
      author,
      review_id,
      created_at,
